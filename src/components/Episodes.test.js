...
test('Renders Episodes when API data is passed down', ()=>{
  const episodes = render(<Episodes episodes={[]}/>)

  episodes.rerender(<Episodes episodes={strangerThings}/>)

  strangerThings.forEach(episode =>{
    expect(episodes.getByText(episode.name)).toBeInTheDocument()
  })
})